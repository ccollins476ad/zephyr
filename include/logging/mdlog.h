...
/* Log system level functions (for all mdlogs.) */
const char *mdlog_module_name(u8_t module_id);
const char *mdlog_level_name(u8_t level_id);

void mdlog_init(void);
struct mdlog *mdlog_get_next(struct mdlog *);

/* Log functions, manipulate a single mdlog */
...
int mdlog_read(struct mdlog *mdlog, const void *src, void *buf, u16_t off,
               u16_t len);
