 ...
 */
int imgr_ver_str(const struct image_version *ver, char *dst);

/*
 * Given flash_map slot id, read in image_version and/or image hash.
 */
int imgr_read_info(int image_slot, struct image_version *ver, uint8_t *hash, uint32_t *flags);
...
int imgmgr_find_best_slot(void);
int imgmgr_group_register(void);
