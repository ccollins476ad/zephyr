...
/**
 * Indicates whether the specified mdlog has been regiestered.
 */
static bool
mdlog_registered(struct mdlog *mdlog)
{
	struct mdlog *cur;

	for (cur = mdlog_list; cur != NULL; cur = cur->l_next) {
		if (cur == mdlog) {
			return true;
		}
	}
 ...
 */
int
mdlog_read(struct mdlog *mdlog, const void *src, void *buf, uint16_t off,
           uint16_t len)
{
	return mdlog->l_handler->read(mdlog, src, buf, off, len);
}
